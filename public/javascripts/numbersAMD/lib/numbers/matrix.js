...
// _j methods
// Scalar multiplication on an matrix while base matrix is safe
...
matrix.subtract = function (arrA, arrB) {
    if ((arrA.length === arrB.length) && (arrA[0].length === arrB[0].length)) {
        var result = new Array(arrA.length);

        if (!arrA[0].length) {
            // These arrays are vectors.
            for (var i = 0; i < arrA.length; i++) {
                result[i] = arrA[i] - arrB[i];
            }
        } else {
...
matrix.getRow = function(M, n) {
    var result = [];
    for (var i = 0, len = M[n].length; i < len; i++) {
        result.push(M[n][i]);
    }
    return result;
}
...
matrix.getColUnSafe3x = function(M, n, k) {
  var result = [];
  if (n < 0 || n >= M[0].length) {
    throw new Error('The specified n must be between 0 and the number of columns - 1.');
  }
  if (k < 0 || k >= M[0][n].length) {
    throw new Error('The specified k must be between 0 and the number of columns - 1.');
  }
  for (var i=0; i<M.length; i++) {
    result.push(M[i][n][k]);
  }
...
// Make single operation with every element of array
matrix.operate = function(arr, func){
    var result = new Array(arr.length);
...
// Make Math.abs operation for matrix
matrix.abs = function(arr){
    return matrix.operate(arr, Math.abs);
}

// Make Math.abs operation for matrix
matrix.negative = function(arr){
    function negativate(value) { return -value;}
    return matrix.operate(arr, negativate);
}

matrix.vectorTranspose = function (arr) {
    var result = new Array(arr.length);
